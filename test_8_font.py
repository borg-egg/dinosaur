...
init_game()	
#
fontObj = pygame.font.Font('Comic Sans MS', 36)
...
while True: # the main game loop
	...
	if intro_running == True:
		...
		textSurfaceObj = fontObj.render('Gelaufene Meter:  ' + str(int(switch)) , True, COUNTER_COLOR)
		...
		for event in pygame.event.get():										### neu ### 
			if event.type == pygame.MOUSEBUTTONDOWN:
				...
				x = pos[0]
				y = pos[1]
				if x in range(24, 515) and y in range(201, 575):
					init_game()
				elif x in range (592, 992) and y in range (501, 836):
					pygame.quit()
					sys.exit()
